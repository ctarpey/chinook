...
dim(chin_GTseq_ONLY_popINFO)
chin_GTseq_ONLY_popINFO[1:5,1:5]
...
test_set <- chin_GTseq_ONLY_rmvdup[6,-1]
...
for (i in 2:dim(chin_GTseq_ONLY_rmvdup2_rmv847)[2]){
  templocus<-chin_GTseq_ONLY_rmvdup2_rmv847[,i]
  countalleles<-unique(templocus)
  if (countalleles > 5){
    list_to_dump[[i]] <- colnames(chin_GTseq_ONLY_rmvdup2_rmv847[i])
  } else{
    next()
  }
}
...
###### Merge the Pop info with the genotypes ######
#assign each sample to a population 
chin_GTseq_pop<-merge(chin_GTseq_ONLY_popINFO,chin_GTseq_ONLY_rmvdup2_rmv847, by="SampleName")
...
nloci <- length(colnames(chin_GTseq_ONLY_rmvdup2_rmv847))-1
...
colnames(GTseq_BREG_MAF)<-colnames(chin_GTseq_ONLY_rmvdup2_rmv847)[-1]
dim(GTseq_BREG_MAF)
